...
| 0025 | [flip-atomicity-and-reconciliation](0025-flip-atomicity-and-reconciliation.md) — flip (close→open) = due market order sequenziali **non atomici** chain↔DB; **nessuna riconciliazione** DB↔`clearinghouseState` (`_check_pending_closures` rileva solo SL/TP, non il confronto inverso); limite noto accettabile per smoke M6, **riconciliazione BLOCCANTE pre-M7** (dataset); direzione fix tracciata (confronto a inizio tick), non implementata (chiude P3 di ADR-0030) | accepted | 2026-07-07 | M5-T14/M7 | none |
