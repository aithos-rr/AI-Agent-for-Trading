...
| 0039 | [experiment-scoped-open-position-lookups](0039-experiment-scoped-open-position-lookups.md) — fix **root-cause cross-experiment leakage** (gate M6.2 rosso). `list_open_for_model` filtrava `model_id + closed_at IS NULL` **senza** `experiment_id`: per il DB "aperta" era uno stato globale, quindi le righe lasciate aperte da M6.1 (exp `5555…`, *annotate not repair* ADR-0035) erano visibili allo smoke (exp `6666…`, stessi `model_id`/wallet). Prova: `f2207b7b-…` (cn-cheap ETH, aperta 27/07 in M6.1) chiusa il **29/07 18:30 UTC** da una `decision_action` dello smoke con exit 1909.60 e PnL −4.60 (entry vecchia × exit nuova). Cascata: **shift FIFO** — il FLAT prende la prima riga aperta per `(model, symbol)`, chiude quella archiviata, e da lì **ogni** chiusura è sfasata di una posizione (~147 righe `model_close` contaminate) lasciando 1 orfana per `(model,symbol)` attivo = **7 zombie permanenti** irrecuperabili dal reconciler (il FLAT ha chiuso la posizione sul venue → HL **cancella i trigger** → i loro `oid` non compaiono mai in `user_fills`); stessa lettura avvelenava la detection ADR-0025 (8 righe M6.1 → `ChainDivergence` dal tick 1). ADR-0038 **funziona**: il bug è a monte. Fix a livello **repository**: `experiment_id` **obbligatorio e keyword-only** (la vecchia chiamata posizionale non compila: mypy + `TypeError`) + `ORDER BY opened_at, id` esplicito; 3 call-site corretti (`_execute_actions` FLAT, `_reconcile_chain_state`, `_close_for_model` — `_models_with_open_positions` e `FundingReconciler` erano già scoped). Effetto voluto: le righe di esperimenti archiviati sono invisibili **per costruzione** → *annotate not repair* diventa permanentemente sicura. Righe contaminate dello smoke **non** riparate (dataset throwaway, re-smoke su experiment nuovo). Residuo: il wallet HL è condiviso → **nuovo esperimento parte da wallet flat**. 4 test mutation-proof; nessuna migration | accepted | 2026-08-04 | M6.2 | none |
