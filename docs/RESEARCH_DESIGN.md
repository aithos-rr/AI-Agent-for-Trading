...
> *(*) 2026-09-06, ADR-0040 — aggiornamento di aliquota (ADR-0033). Il **26% resta a testo come valore pre-registrato**; l'aliquota **realmente applicata** dalla simulazione è **0.33**. Motivazione normativa: il regime italiano sulle plusvalenze da **cripto-attività** porta l'aliquota al **33% a decorrere dal 2026**, ed è il regime che questo studio assume applicabile al caso in esame (derivati perpetui con leva). Implementazione verificata: `ContextOrchestratorSettings.tax_rate_pct = Decimal("0.33")` (`src/aiat/config/settings.py:131`, overridabile via `AIAT_TAX_RATE_PCT`), passata esplicitamente dal writer a **ogni** riga di simulazione (`src/aiat/orchestration/tax_sim_runner.py:127`); il `server_default` 0.26 dello schema resta invariato e non viene mai usato (nessuna migration). Tutto ciò che questa nota dichiara **non tocca le riserve sopra**: resta una metrica sperimentale controfattuale, non consulenza fiscale né un calcolo legale, e resta un **parametro di configurazione** — l'analisi è ricalcolabile con un'aliquota diversa senza toccare i dati grezzi. Le altre occorrenze del 26% nel documento (§3.3, §4.1, §5) rimandano a questa nota.*
...
- `PnL_netto-fee-funding-tax-sim` (post 26% simulazione fiscale ITA — *(*) 2026-09-06, ADR-0040: aliquota operativa **0.33**, vedi la nota terminologica in §1.1*)
...
> *(*) 2026-09-06, ADR-0040 — nota di allineamento (ADR-0036): i tre baseline restano quelli pre-registrati qui, con **questi** parametri, che non cambiano. Cambia solo il **momento del calcolo**: non più a posteriori a fine esperimento, ma **live a ogni tick di 15 minuti**, calcolati dal `context-orchestrator` dai prezzi del `context_snapshot` già materializzato (nessuna chiamata esterna aggiuntiva) e persistiti come una riga per baseline e per tick; il calcolo a posteriori resta disponibile come backfill/catch-up e rigioca la stessa identica logica. Due precisazioni operative fissate insieme al calcolo live, da leggere accanto ai parametri elencati sotto: gli SL/TP della strategia naive sono valutati sul **close** del tick — i wick intra-candle non sono catturati (approssimazione conservativa) e ne segue un'**asimmetria dichiarata** rispetto agli SL/TP dei modelli LLM, che scattano intra-tick on-chain; la fee taker applicata alla strategia naive è `0.00045` (4.5 bps), validata empiricamente sui `fee_events` reali dell'infrastruttura.*
...
> *(*) 2026-09-06, ADR-0040 (ADR-0033): il nome `tax_sim_26pct(...)` è **mantenuto come scritto in pre-registrazione** ma va letto come `tax_sim_rate(...)` con **`rate = 0.33`** — motivazione normativa nella nota terminologica di §1.1. Precisazione operativa per chi scriverà le query: la simulazione fiscale è persistita come **aggregato di periodo** (una riga per modello e per periodo: `taxable_base = max(0, gross − fees − funding)`, `tax_due = taxable_base × rate`, con la compensazione algebrica perdite/profitti del regime semplificato), mentre la colonna **per-outcome** `pnl_net_fee_funding_tax_sim_usd` resta `0` by design. La variabile `PnL_netto-fee-funding-tax-sim` di §3.3 si ottiene quindi combinando gli outcome con l'aggregato di periodo, **non** leggendo quella colonna.*
...
- I 4 modelli **non genereranno tutti PnL netto post-tax positivo**. Il mercato crypto-perpetuals è efficiente al limite per agenti retail, e i costi (fee + funding + 26% tax) sono significativi. Aspetto 0-2 modelli con PnL netto leggermente positivo, 2-4 con PnL netto leggermente negativo. *(*) 2026-09-06, ADR-0040: l'aspettativa resta **invariata come pre-registrata**; l'aliquota realmente applicata è 0.33 (nota in §1.1), quindi il costo fiscale simulato è **maggiore** di quello ipotizzato qui e l'aspettativa risulta, semmai, più conservativa — non più facile da confermare.*
...
12. **Il determinismo non è uniforme tra i soggetti.** Il disegno prescriveva `temperature = 0` + `seed = 42` per tutti e quattro i modelli (ADR-0020), ma i modelli Anthropic *thinking-only* **rifiutano** `temperature` (HTTP 400 osservato sull'API reale) e non espongono `seed`/`top_p`: il modello `usa-premium` gira quindi nel proprio regime nativo, **non-deterministico per costruzione**, mentre gli altri tre girano con i sampling parameter fissati. Conseguenze: la riproducibilità esatta del singolo run non è garantita per tutti i soggetti, e la varianza intra-modello va trattata in modo **provider-aware** nell'analisi. Inquadramento dichiarato: i modelli sono confrontati nel loro **regime reale di esercizio** (condizione ecologica — è così che opererebbero in produzione), non in un regime artificiale che alcuni provider non supportano. Il campo `temperature` persistito registra il valore **realmente usato** (`None` per Anthropic), non un falso `0`. *(*) 2026-09-06, ADR-0040: voce aggiunta per propagare ADR-0023.*
13. **Varianza residua dello structured output anche sotto `temperature = 0` + `seed`.** Sul provider diretto OpenAI (`gpt-4.1-mini`) il percorso `json_schema` produce occasionalmente un output che **non valida al primo colpo** ed è recuperato dall'unico fallback freetext pre-registrato: ~2 invocazioni su 8 nella verifica su prompt reale, con 0 fallimenti irrecuperabili. Due conseguenze da dichiarare: (i) il determinismo è **best-effort anche dove i sampling parameter sono accettati** — la limitazione 12 non riguarda solo Anthropic; (ii) una quota di decisioni (~25% sul campione osservato) nasce da `prompt + FALLBACK_SUFFIX`, cioè da un prompt **leggermente diverso** da quello nominale di cui §3.2 registra l'hash. Per questo `fallback_used` è raccolto per-modello e trattato come **metrica di robustezza/conformità**, non come errore da azzerare. La stima è su campione piccolo (8 invocazioni, un solo modello) e va raffinata sui volumi dell'esperimento. *(*) 2026-09-06, ADR-0040: voce aggiunta per propagare ADR-0028.*
14. **La schema-compliance è una variabile comportamentale misurata, non un difetto corretto a posteriori.** Un tick la cui response non supera la validazione dello schema `TradeDecision` né al tentativo primario né all'unico fallback freetext termina come run fallita (`failure_stage = 'llm_parse'`) e **non viene rigiocato né ri-promptato**: un retry darebbe più tentativi al modello che sbaglia di più, cioè un trattamento sperimentale **asimmetrico** — confound diretto per RQ2 e RQ3. Conseguenze per l'analisi: (i) il denominatore delle metriche di affidabilità **esclude** gli schema-failure (criterio ≥95% di run riuscite per modello), con **soglia dedicata ≥85% inclusiva** per il modello che porta il tasso più alto di fallimenti irrecuperabili (~10%, `usa-cheap`), così che la compliance entri esplicitamente nel giudizio di fattibilità di quel modello; (ii) la variabile misurata è la **capacità di produrre output utilizzabile entro il protocollo pre-registrato** (structured output + un fallback), non la compliance stretta al primo colpo — che resta osservabile separatamente via `fallback_used`; (iii) il costo API dei tentativi falliti non entra nel cost ledger (sotto-conteggio minore, dichiarato). I retry di trasporto su 5xx/429 restano attivi e **simmetrici** fra i quattro modelli: la politica no-retry riguarda solo la validazione. *(*) 2026-09-06, ADR-0040: voce aggiunta per propagare ADR-0037.*
15. **Il Brier score copre le sole decisioni direzionali.** La regola di labeling controfattuale per HOLD/FLAT (§2.1) è decisa, implementata e testata — `OutcomeResolver.resolve_hold_flat`, regola *fee-hurdle*: HOLD/FLAT è "corretto" se `|Δprezzo%| ≤ fee_roundtrip_pct` sull'orizzonte dichiarato — ma **non ha alcun chiamante di produzione**: le righe `outcomes` sono scritte esclusivamente alla chiusura di una posizione realmente aperta. Le decisioni HOLD e FLAT, attese al **30-50%** del totale (§6.1), **non producono quindi alcun outcome**, e la calibrazione di confidence di RQ2.2 è misurata sul solo sottoinsieme direzionale, con **bias sistematico a favore dei modelli che tradano di più**. È esattamente l'alternativa (esclusione di HOLD/FLAT dal Brier) che la decisione su D2 aveva scartato: va dichiarato, non aggirato. Il calcolo su HOLD/FLAT resta **possibile offline** in fase di analisi, ricostruendo `price_at_decision` e `price_at_horizon` dai `context_snapshots` e applicando la stessa regola fee-hurdle, che resta valida. *(*) 2026-09-06, ADR-0040: voce aggiunta; il non-cablaggio è una decisione esplicita per l'esperimento (M7), non una svista.*
