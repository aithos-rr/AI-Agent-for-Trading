...
(a) **Copertura**: le fee reali da `user_fills` sono persistite solo dal commit 51a8e45 (~11/07): le chiusure precedenti (~190 outcomes) hanno `sum_fees_usd=0`. Decisione backfill: [DA COMPLETARE al gate — script `backfill_fees.py` pronto, dry-run].
